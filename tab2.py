"""
tab2.py: Award Chart Lookup - CORRECTED

Key changes:
1. Filter carriers by operating_region CODE (e.g., "US", "US-EU") matching route_category key
2. Use origin_region/destination_region to filter locations from zone_definitions
3. Match strings correctly:
   - route_categories KEY (e.g., "US") matches carrier operating_region
   - origin_region/destination_region values match zone_definitions keys
"""
...
class Tab2Frame(ttk.Frame):
    ...
    def _setup_route_details(self):
        ...
        origin_dropdown = ttk.Combobox(self.details_frame, textvariable=self.origin_var,
                                        values=origin_locations, state="readonly", width=25)
        ...
        dest_dropdown = ttk.Combobox(self.details_frame, textvariable=self.dest_var,
                                      values=dest_locations, state="readonly", width=25)
